...
corr <- function(directory, threshold = 0) {
    ...
    for (i in 1:332) {
        files <- read.csv(fullfiles[i]) 
        comp <- files[complete.cases(files),]
        if (nrow(comp) > threshold) {
            dat <- c(dat, cor(comp[["sulfate"]], comp[["nitrate"]]))
        }
    }
    dat
}
```
The part 3 was really difficult for me. My solution was inspired by the article from https://zenagiwa.wordpress.com/2016/06/08/r-programming-assignment-1/
