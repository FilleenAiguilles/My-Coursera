Data--The zip file containing the data can be downloaded here: https://d396qusza40orc.cloudfront.net/rprog%2Fdata%2Fspecdata.zip
...
complete1 <- function(directory, id= 1: 332) {
    ...
    for (i in id) {
        files <- read.csv(fullfiles[i])
        nrow <- nrows(files[complete.cases(files),] )  
        nobs <-c(nobs, nrow)
    }
...
```
My third solution was inspired by the article from https://zenagiwa.wordpress.com/2016/06/08/r-programming-assignment-1/
